    ...
    if 'networks' in data:
        if 'ipv6' in data['networks']:
            for net_str in data['networks']['ipv6']:
                ...
                net_prefix = net.network_address \
                                .exploded \
                                .replace(":", "")[:net_length]
        ...
        for domain in data['domains']:
            if domain.endswith('.ip6.arpa') or domain.endswith('.in-addr.arpa'):
                ...
                for reverse_domain in allowed_domains:
                    # we want communitys to be able to announce only a part of the net as RDNS-capable
                    if domain.endswith(reverse_domain):
                        found = True

                if not found:
                    errcnt += 1
                    error("Illegal RDNS-Domain: found no matching net for %s (found nets: %s)" % (
